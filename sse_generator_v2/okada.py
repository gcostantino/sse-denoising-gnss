...
def plot_components(x, y, ux, uy, uz, los, params, profile=False):
    ...
    fig, (ax, ax1, ax2) = plt.subplots(1, 3,
                                       subplot_kw=dict(aspect=1.0, adjustable='box'),
...
def plot_los(x, y, ux, uy, uz, los, params, profile=False):
    ...
    # --------------
    fig, (ax, ax1, ax2) = plt.subplots(1, 3, subplot_kw=dict(aspect=1.0, adjustable='box'), sharex=True,
    ...
    cb = plt.colorbar(im, ax=ax2, orientation='horizontal', ticks=[],
                      pad=0.1)  # ticks=MaxNLocator(nbins=5) #5 ticks only)
...
def plot_profile(ind, ux, uy, uz, los, axis=0):
    ...
    if axis == 0:
        ...
        for elem in ind:
            plt.axhline(elem.all(), color='k', lw=2)
    ...
    for a in (ax, ax1, ax2, ax3):
        ...
        for elem in ind:
            a.axvline(elem.all(), color='gray', linestyle='--')
        a.grid(True)
